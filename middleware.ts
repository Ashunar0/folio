...
export async function middleware(request: NextRequest) {
  ...
  if (user && !justJoined && (path.startsWith("/app/") || path.match(/^\/[a-f0-9-]{36}\//))) {
    ...
    if (teamId) {
      ...
      // These routes require admin or manager role
      const protectedRoutes = ["/approval", "/users", "/settings", "/categories"];
      const currentRoute = path.replace(`/${teamId}`, "");
      ...
      if (isProtectedRoute) {
        ...
        if (!allowedRoles.includes(userRole)) {
          ...
          const redirectUrl = request.nextUrl.clone();
          redirectUrl.pathname = `/${teamId}/dashboard`;
          return NextResponse.redirect(redirectUrl);
        }
      }
    }
