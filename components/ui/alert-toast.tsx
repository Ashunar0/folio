import { Alert, AlertTitle, AlertDescription } from "@/components/ui/alert";
import {
  OctagonAlert,
  AlertTriangleIcon,
  CircleFadingArrowUpIcon,
  CircleCheckBigIcon,
} from "lucide-react";

interface AlertProps {
  title: string;
  description?: string;
}

export const DestructiveAlert = ({ title, description }: AlertProps) => {
  return (
    <Alert className="bg-destructive/10 dark:bg-destructive/15 text-destructive border-destructive/30 dark:border-destructive/50">
      <OctagonAlert className="size-4" />
      <AlertTitle>{title}</AlertTitle>
      {description && (
        <AlertDescription className="text-destructive/90">
          {description}
        </AlertDescription>
      )}
    </Alert>
  );
};

export const AlertWarning = ({ title, description }: AlertProps) => {
  return (
    <Alert className="bg-amber-600/10 dark:bg-amber-600/15 text-amber-500 border-amber-500/50 dark:border-amber-600/50">
      <AlertTriangleIcon className="size-4" />
      <AlertTitle>{title}</AlertTitle>
      {description && (
        <AlertDescription className="text-amber-500/90">
          {description}
        </AlertDescription>
      )}
    </Alert>
  );
};

export const AlertInfo = ({ title, description }: AlertProps) => {
  return (
    <Alert className="bg-blue-500/10 dark:bg-blue-600/20 text-blue-500 dark:text-blue-400 border-blue-400/50 dark:border-blue-600/60">
      <CircleFadingArrowUpIcon className="size-4" />
      <AlertTitle>{title}</AlertTitle>
      {description && (
        <AlertDescription className="text-blue-500/90">
          {description}
        </AlertDescription>
      )}
    </Alert>
  );
};

export const AlertSuccess = ({ title, description }: AlertProps) => {
  return (
    <Alert className="bg-emerald-600/10 dark:bg-emerald-600/15 text-emerald-500 border-emerald-500/50 dark:border-emerald-600/50">
      <CircleCheckBigIcon className="size-4" />
      <AlertTitle>{title}</AlertTitle>
      {description && (
        <AlertDescription className="text-emerald-500/90">
          {description}
        </AlertDescription>
      )}
    </Alert>
  );
};
